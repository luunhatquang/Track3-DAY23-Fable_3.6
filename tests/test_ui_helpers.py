...
def test_normalize_events_fills_missing_optional_fields() -> None:
...
def test_normalize_events_handles_none_and_empty() -> None:
    assert normalize_events(None) == []
    assert normalize_events([]) == []


def test_normalize_events_preserves_metadata_and_latency() -> None:
    raw = [{
        "node": "tool",
        "event_type": "error",
        "message": "boom",
...
def test_render_status_badge_has_text_label_not_only_color() -> None:
    html = render_status_badge("completed")
    assert "Completed" in html
    assert "badge-success" in html


def test_render_status_badge_unknown_status_falls_back() -> None:
    html = render_status_badge("weird_status")
    assert "Weird Status" in html
    assert "badge-muted" in html


def test_session_defaults_and_reset_keys_never_touch_graph_or_checkpointer() -> None:
...
def test_build_checkpoint_rows_reads_step_node_ts() -> None:
    tuples = [
        _FakeCheckpointTuple(
            {"id": "chk-1", "ts": "2026-08-25T10:00:00Z"},
            {"step": 0, "source": "input"},
        ),
        _FakeCheckpointTuple(
            {"id": "chk-2", "ts": "2026-08-25T10:00:01Z"},
...
def test_build_checkpoint_rows_handles_missing_metadata() -> None:
    tuples = [_FakeCheckpointTuple({"id": "chk-1", "ts": ""}, {})]
    rows = build_checkpoint_rows(tuples)
    assert rows == [{"step": 0, "node": "chk-1", "ts": ""}]


def test_build_checkpoint_rows_empty_input() -> None:
