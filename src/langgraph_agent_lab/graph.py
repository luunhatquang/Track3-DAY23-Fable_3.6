...
def build_graph(checkpointer: Any | None = None):
    ...
    from .routing import (
        route_after_approval, route_after_classify, route_after_evaluate, route_after_retry,
    )
    ...
    for name, node in {
        ...
        "retry": retry_or_fallback_node, "dead_letter": dead_letter_node,
        "finalize": finalize_node,
    }.items():
        builder.add_node(name, node)
    for source, target in (
        (START, "intake"), ("intake", "classify"), ("tool", "evaluate"),
        ("answer", "finalize"), ("clarify", "finalize"), ("risky_action", "approval"),
        ("dead_letter", "finalize"), ("finalize", END),
    ):
